...
<!-- Tombol Tambah -->
<a type="button" class="btn btn-primary btn-sm m-1 py-0" href="?dir=user&file=insert">Tambah</a>
	...
	<tbody>
		...
		<?php foreach ($user as $usr) : ?>
			<tr>
				...
				<td><?= $usr["user"]; ?></td>
				<td><?= $usr["email"]; ?></td>
				<td><?= strtoupper($usr["posisi"]); ?></td>
				<td class="text-center">
					<?php if ( $usr["status"] == 1 ) : ?>
						<a href="?dir=user&file=update-status&id=<?= $usr["iduser"] ?>&status=<?= $usr["status"] ?>" class="btn btn-success btn-sm m-0 py-0">
							AKTIF
						</a>
					<?php else : ?>
						<a href="?dir=user&file=update-status&id=<?= $usr["iduser"] ?>&status=<?= $usr["status"] ?>" class="btn btn-danger btn-sm m-0 py-0">
							BANNED
						</a>
					<?php endif; ?>
				</td>
				<td class="text-center">
					<a href="?dir=user&file=confirm-delete&id=<?= $usr["iduser"] ?>" class="btn btn-outline-danger btn-sm m-0 py-0">
						Hapus
					</a>
				</td>
			</tr>
